package _01_solid.DIP;

/*

    NOTES:
    - Dependency Inversion:
        - This means we should not be having any dependencies on any modules or any sub-modules; meaning high level modules should not be having dependency on low level modules or any classes or objects of it. 
    ...
    - BAD EXAMPLE EXPLANATION:
        - Calculator interface:
            - Is having calculate method() where we have a swith statement for passing the operation we want, if we pass the "add" we are creating the AddOperation (using the new keyword) and when we pass the "sub" we are creating the SubOperation too
    ...
    - GOOD EXAMPLE EXPLANATION:
        ...
        - Concrete classes:
        - SubOperation & AddOperation classes:
        ...
        - Calculator class:
            - Here the CalcOperation interface has calculate() method which takes 2 numbers
                    public class Calculator {
        ...
        - When we now come to the Calculator class, we have no dependency, we just pass 2 variables as parameters to calculate() methods and then we are just passing the operation, no object creation and whatever expression needed is passed at CalcOperation and calculated directly
                public class Main {
