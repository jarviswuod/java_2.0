/*
    ...
    - Thread Joining:
        ...
        -  Thread is in a 'ready-to-run" state, once it's given priority to execute and you call the join() methd, you are simply pausing the asynchroous nature of thread execution and to a 'Blocked-for-join-completion' state.
    ...
    - Thread Priorities:
        - Threads are assigned priorities that the thread scheudler can use to determine how the threads will be scheduled
        - The thread scheduler can use thread priorities to determine which thread gets to run
        - Priorities are interget values from 1(lowest priority) to 10(highest priority) with default being 5;
            - Thread.MIN_PRIORITY
            - Thread.MAX_PRIORITY
