/*
    ...
    NOTES:
        - We have a queue and 2 groups of threads, One group of threads is trying to add an item to the queue, the other group of threads is trying to remove items from the queue
        - We can only push items to the queue if it doesn't increase the capacity and we can pull items in the queue if there is an item left in the queue.
        - Breakdown;
            - Multiple threads adding items to the queue
            - Multiple threads removing items to the queue
 
        - Code Explanation:
            - Since multiple threads are trying to access our queue, we introduce the synchronized block:
                - synchronized (q) {...}

            - We have the option of using 'this' as our lock but we will use 'q' since an Object of the Queue class
            - Since we introduced the 'synchronized' keyword only one thread is allowed to access either of the methods
